...
static int askled_probe(struct platform_device *dev)
{
    ...
    if(len == 32)
    {
        printk("Cannot load more device\n");
        return -1;
    }

    err = of_property_read_u32(dev->dev.of_node, "pin", minorTopin + len);
    if(err < 0)
    {
        printk("ERROR : %s %s %d\n", __FILE__, __FUNCTION__, __LINE__);
        return -1;
    }
    led_device_create(len++);
...
static int askled_remove(struct platform_device *dev)
{
    ...
    err = of_property_read_u32(dev->dev.of_node, "pin", &gp);
    if(err < 0)
    {
        printk("It is not tree deviec\n");
        printk("%s %s %d\n", __FILE__, __FUNCTION__, __LINE__);
