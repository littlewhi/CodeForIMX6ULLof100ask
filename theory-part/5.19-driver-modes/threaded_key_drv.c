...
static irqreturn_t yhb_key_thread_func( int irq, void *data )
{
    struct yhb_gpio_key *key = (struct yhb_gpio_key *)data;

    int val = gpio_get_value( key->gpio );
    printk( "%s : key %d val is %d\n", __FUNCTION__, key->gpio, val);
...
static int yhb_key_probe( struct platform_device *pdev )
{
    ...
    for( i = 0; i < count; ++i )
    {
        ...
        if( request_threaded_irq( irq, yhb_key_interrupt, yhb_key_thread_func,
